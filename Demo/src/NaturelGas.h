...
class NaturelGas : public QObject
{
    Q_OBJECT
    QML_ELEMENT
...
public:
    explicit NaturelGas(QObject * parent = nullptr);
    Q_INVOKABLE void loadingPriceNatGas();
    QUrl url() const;
...
private:
    ...
    std::shared_ptr<QRestAccessManager> manager;
    std::shared_ptr<QNetworkRequestFactory> api;
    QUrl m_url = QUrl("https://www.alphavantage.co/query?function=NATURAL_GAS&interval=monthly&apikey=demo");
    QString m_NameProd;
    QString m_interval;
    QString m_unit;
    QStringList m_date;
